...
Topic *ServerTopicRepository::loadTopic(char *topic) {
    ...
    long subscribedUserCount = j.at("subscribed_users_count").get<int>();
    long lastMsgIdPublished = j.at("last_msg_id_published").get<int>();
...
//Message *ServerTopicRepository::loadMessage(Topic *topic, int msgId) {
//    return nullptr;
//}


std::vector<ServerMessageContainer *> *ServerTopicRepository::consumeMessagesStartingFromId(char *topic, int lastConsumedMsgId) {
...
std::vector<ServerMessageContainer *> *ServerTopicRepository::loadMessages(char *topicName) {
    ...
    for (json::iterator it = j.begin(); it != j.end(); ++it) {
        int msgId = it.value().at("id").get<int>();
        int unconsumed_user_count = it.value().at("unconsumed_user_count").get<int>();
